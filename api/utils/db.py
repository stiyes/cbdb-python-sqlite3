...
def query_db_many(db_file, table, fields):
    ...
    with db_cursor(db_file, commit=False) as cursor:
        return cursor.execute(f"SELECT * FROM {table} where {named_fields_clause}", fields).fetchmany(10)
...
def query_inner_join(db_file, table_left, table_right, field_key_left, field_key_right, field_filters_left,
    ...
    print(f"SELECT * FROM {table_left} t1 INNER JOIN {table_right} t2 on t1.{field_key_left} = t2.{field_key_right} "
            f"where {named_fields_clause}", field_filters)
